...
const RemindersPage = () => {
  ...
  const sendEmailRemind = async (customer) => {
    ...
    setSending(p => ({ ...p, [customer._id + '_email']: true }));
    try {
      ...
      setSending(p => ({ ...p, [customer._id + '_email']: false })); 
    }
  };
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <button 
          className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-orange hover:bg-orange-hover text-pure-white rounded-xl text-sm font-bold border-none cursor-pointer transition-all shadow-sm hover:shadow disabled:opacity-50 disabled:cursor-not-allowed" 
          onClick={sendBulk} 
                ...
                <tr key={c._id} className="border-b border-soft-gray/50 hover:bg-light-cream/10 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm align-middle">
                    <div className="flex flex-wrap items-center gap-2">
                      <button 
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange hover:bg-orange-hover text-pure-white text-xs font-bold rounded-lg border-none cursor-pointer transition-all shadow-2xs hover:shadow-xs disabled:opacity-30 disabled:cursor-not-allowed" 
                        title="Send Email Reminder" 
                        onClick={() => sendEmailRemind(c)}
                        disabled={sending[c._id + '_email'] || !c.email}
                      >
                        {sending[c._id + '_email'] ? (
